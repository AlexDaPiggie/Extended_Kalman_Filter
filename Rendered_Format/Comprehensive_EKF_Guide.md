...
### 2. Measuring Uncertainty (Variance and Covariance) - Continued
...
### Step 1: The Prediction - Continued
...
* **Algebra Format (1D):**
  $$K = \frac{\text{Prediction Uncertainty}}{\text{Prediction Uncertainty} + \text{Sensor Noise (R)}} = \frac{p \cdot h}{h \cdot p \cdot h + R}$$
  Final State = Prediction + $K \cdot (\text{Actual Sensor} - \text{Expected Sensor})$
  $$x_{final} = x_{new} + K \cdot (z - h \cdot x_{new})$$

---

### Step 2: The Correction - Continued
...
* **Advanced Math (Linear Algebra) Format:**
  ...
  (where $I$ is the identity matrix, acting like the number 1).
...
### The Prediction Phase (EKF) - Continued
...
### The Correction Phase (EKF) - Continued
